...
export async function onRequestPost(context) {
  ...
  const res = await fetch(
    `${SUPABASE_URL}/rest/v1/funeral_directors?id=eq.${directorId}&select=photos,pending_photos`,
  ...
  const newPhotos  = [...(director.photos || []), photoUrl]
  ...
  await fetch(
    `${SUPABASE_URL}/rest/v1/funeral_directors?id=eq.${directorId}`,
    {
      ...
      body: JSON.stringify({ photos: newPhotos, pending_photos: newPending }),
    }
  )
